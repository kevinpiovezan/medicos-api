...
describe('FindByBairroService', () => {
  let service: FindByBairroService;

  let medicos: ICreateDTO[];
  const mockRepository = {
    find: jest.fn((bairro) =>
      medicos.find((medico) => medico.bairro === bairro),
    ),
  };
  ...
  beforeEach(async () => {
    ...
    service = module.get<FindByBairroService>(FindByBairroService);
  ...
  it('should return all medicos by bairro', () => {
    expect(mockRepository.find('bairro legal')).toEqual({
      name: 'Mock',
      cep: '01234567',
      localidade: 'São Paulo',
      uf: 'SP',
