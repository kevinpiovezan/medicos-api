...
describe('FindByTelFixoService', () => {
  let service: FindByTelFixoService;

  let medicos: ICreateDTO[];
  const mockRepository = {
    findOne: jest.fn((tel) =>
      medicos.find((medico) => medico.tel_fixo === tel),
    ),
  };
  ...
  beforeEach(async () => {
    ...
    service = module.get<FindByTelFixoService>(FindByTelFixoService);
  ...
  it('should return a medico by telefone Fixoular', () => {
    expect(mockRepository.findOne(1127654321)).toEqual({
      name: 'Mock',
      cep: '01234567',
      localidade: 'São Paulo',
      uf: 'SP',
  ...
  it('should not return a medico if telefone Fixo is not on the database', () => {
